...
export default async function EditTransactionPage({ params }: { params: Promise<{ id: string }> }) { const { id } = await params; const [result, options] = await Promise.all([getTransaction(id), getTransactionOptions()]); if (result.configured && !result.transaction) notFound(); return <TransactionForm accounts={options.accounts} categories={options.categories} configured={result.configured} transaction={result.transaction ?? { ...demoTransaction, id }} />; }
