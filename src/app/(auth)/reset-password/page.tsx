...
export default function ResetPasswordPage() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (password.length < 8) { toast.error("Mật khẩu cần ít nhất 8 ký tự."); return; }
    if (password !== confirmation) { toast.error("Hai mật khẩu chưa khớp."); return; }
    setLoading(true);
    const supabase = createClient();
    const { error } = await supabase.auth.updateUser({ password });
    setLoading(false);
    if (error) { toast.error("Liên kết đã hết hạn hoặc không hợp lệ. Vui lòng yêu cầu lại."); return; }
