...
export default function RegisterPage() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) { event.preventDefault(); setLoading(true); const supabase = createClient(); const { data, error } = await supabase.auth.signUp({ email, password, options: { emailRedirectTo: `${window.location.origin}/auth/callback?next=/app/onboarding` } }); setLoading(false); if (error) { setMessage("Không thể tạo tài khoản. Vui lòng kiểm tra lại thông tin."); return; } if (data.session) { router.push("/app/onboarding"); router.refresh(); return; } setMessage("Kiểm tra email để xác nhận tài khoản của bạn."); }
