...
export function BottomNav({ preview = false, active = "home" }: { preview?: boolean; active?: NavItem }) {
  const href = (path: string) => (preview ? "#" : path);
  const itemClass = (item: NavItem) => cn("flex min-h-11 flex-col items-center justify-center gap-1 text-[11px] font-medium", active === item ? "text-[var(--brand)]" : "text-[var(--muted)]");
