...
export async function getAccounts() {
  ...
  const { data, error } = await supabase.from("accounts").select("id,name,type,initial_balance,current_balance,currency,color,is_archived,include_in_total").eq("user_id", user.id).order("is_archived", { ascending: true }).order("created_at", { ascending: true });
