...
import { ArrowDownLeft, ArrowLeftRight, ArrowUpRight, ChevronLeft } from "lucide-react";
...
export function TransactionForm({ accounts, categories, configured, transaction }: { accounts: AccountRecord[]; categories: CategoryOption[]; configured: boolean; transaction?: TransactionRecord }) {
  ...
  const form = useForm<TransactionFormInput>({
    ...
    defaultValues: transaction ? {
      ...
      merchant: "", note: "", transactionDate: localToday(),
    },
  });
  ...
  return <main className="min-h-screen min-w-0 bg-[var(--background)] px-4 pb-[calc(env(safe-area-inset-bottom)+7rem)] pt-5 sm:px-6 md:px-10 md:pb-10 md:pt-8"><div className="mx-auto max-w-2xl">
    ...
    <Card className="mt-6 p-5 sm:p-7"><form onSubmit={form.handleSubmit(submit)} className="space-y-5">
      ...
      <div className="grid min-w-0 gap-4 sm:grid-cols-2"><label className="block min-w-0 text-sm font-semibold">Ngày giao dịch<DateInput {...form.register("transactionDate")} required className="mt-2" />{form.formState.errors.transactionDate && <span className="mt-1 block text-xs text-[var(--danger)]">{form.formState.errors.transactionDate.message}</span>}</label><label className="block min-w-0 text-sm font-semibold">Nơi giao dịch<input {...form.register("merchant")} className="mt-2 min-h-11 w-full rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 text-base" placeholder="Ví dụ: Highlands" /></label></div>
