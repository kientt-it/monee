...
export function TransactionDetailView({ transaction, accounts, categories, configured }: { transaction: TransactionRecord; accounts: AccountRecord[]; categories: CategoryOption[]; configured: boolean }) {
  const router = useRouter(); const [confirming, setConfirming] = useState(false); const [isPending, startTransition] = useTransition();
  ...
  function remove() { startTransition(async () => { const result = await deleteTransactionAction(transaction.id); if (!result.ok) { const message = result.error === "SUPABASE_NOT_CONFIGURED" ? "Bản xem trước không xóa dữ liệu thật." : result.error; toast.error(message); return; } toast.success("Đã xóa giao dịch. Bạn có thể hoàn tác ở danh sách giao dịch."); router.push(`/app/transactions?deleted=${encodeURIComponent(transaction.id)}`); }); }
